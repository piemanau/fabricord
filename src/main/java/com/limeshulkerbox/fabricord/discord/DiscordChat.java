...
public class DiscordChat extends ListenerAdapter {
    ...
    public void onMessageReceived(@NotNull MessageReceivedEvent event) {
        ...
        // make Discord emojis a bit more readable in-game
        content = content.replaceAll("<:(\\w+):[0-9]+>", ":$1:");

        // useful for sharing screenshots and stuff
        if (event.getMessage().getAttachments().size() > 0) {
            for (Message.Attachment a: event.getMessage().getAttachments()) {
                // todo: make it clickable in-game with net.minecraft.network.chat.Component
        ...
        if (!config.isSendDiscriminatorToMinecraft()) {
            ...
            API.sendMessage(String.format("§9[§f%s%s§9] §f%s", name, reply_author, content), false, false, true);
        } else {
    ...
    private String getList() {
        ...
        formattedString.append(String.format("%o/%o players online", API.getServerVariable().getCurrentPlayerCount(), API.getServerVariable().getMaxPlayerCount()));
